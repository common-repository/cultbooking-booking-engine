...
if ( ! class_exists( 'CultBooking_Hotel_Booking_Engine_Channel_Manager' ) ) {
	...
	class CultBooking_Hotel_Booking_Engine_Channel_Manager {
		...
		public function chbecm_enqueue_front(){
			...
			wp_enqueue_script ('chbecm-iframe-js', CHHECM_URL . '/assets/js/iframe.js', 'jquery' );
		}
		...
		public function chbecm_iframe_plugin_add_shortcode_fun($atts){
			...
			$st = shortcode_atts($default, $atts);
			$src = 'https://neo.cultbooking.com/CPC/iframe.php?agentcode=58078&hotelcode=' . $chbecm_hotel_id;
			$height = intval($st['height']) . 'px';
			$zindex = intval($st['z_index']);
			$top = intval($st['margin_top']) . 'px';
						
			if($st['view'] === 'fullPage') {
				$src = 'https://neo.cultbooking.com/CPC/?agentcode=58078&hotelcode=' . $chbecm_hotel_id;
			} else if($st['view'] === 'iFrame'){
				$src = 'https://neo.cultbooking.com/CPC/iframe.php?agentcode=58078&hotelcode=' . $chbecm_hotel_id;
			}
			
			if (preg_match('/^\d{1,20}(?:,\d{1,20})*$/', $st['room_type'])) {
				$src .= '&rc=' . $st['room_type'];
			}
			if (preg_match('/^\d{1,20}(?:,\d{1,20})*$/', $st['product_code'])) {
				$src .= '&pc=' . $st['product_code'];
			}
			
			if (preg_match('/^[a-zA-Z]{2}$/', $st['lang'])) {
				$src .= '&lang=' . $st['lang'];
			}
			
			if ($st['calendar'] === 'sellableUnit') {
				$src .= '&calendar=true';
			}
			...
			if(isset($chbecm_hotel_id)) {
				...
				$html .= '<style>iframe.chbecm-booking-engine{width: 100% !important;}</style>
			                ...
			                <tr>
			                    ...
			                    <td>
			                        <p>
			                            ...
			                            <span><a href='<?php echo esc_url_raw( "https://www.cultbooking.com/en/cultbooking-plugin-documentation/" ); ?>' target="_blank"> <?php echo __( 'Visit The CultBooking Documentation', 'cultbooking-booking-engine' ); ?></a></span>
